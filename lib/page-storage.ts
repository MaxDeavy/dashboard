...
export function readStoredAdminServicesPageId(): number | null {
  if (typeof window === "undefined") return null;

  try {
    const stored = localStorage.getItem(ADMIN_SERVICES_PAGE_STORAGE_KEY);
    if (!stored) return null;
    const parsed = Number(stored);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
  } catch {
...
export function writeStoredAdminServicesPageId(pageId: number) {
  if (typeof window === "undefined") return;

  try {
    localStorage.setItem(ADMIN_SERVICES_PAGE_STORAGE_KEY, String(pageId));
  } catch {
    // ignore
  }
}
